...
int serialport_write(const unsigned char *buf, unsigned int writecnt)
{
	...
	while (writecnt > 0) {
#if IS_WINDOWS
		if (WriteFile(sp_fd, buf, writecnt, &tmp, NULL) != true) {
...
int serialport_read(unsigned char *buf, unsigned int readcnt)
{
	...
	while (readcnt > 0) {
#if IS_WINDOWS
		if (ReadFile(sp_fd, buf, readcnt, &tmp, NULL) != true) {
...
#if IS_WINDOWS
		if (ReadFile(sp_fd, c + rd_bytes, readcnt - rd_bytes, &rv, NULL) != true) {
...
#if IS_WINDOWS
		if (WriteFile(sp_fd, buf + wr_bytes, writecnt - wr_bytes, &rv, NULL) != true) {
